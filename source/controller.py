...
class ControllerInfo:
    def __init__(self):
        ...
        self.requeue_chunks = deque()  # O(1) deque for requeue
...
def parse_arguments(controller_info):
    ...
    parser.add_argument("-b", "--interval", type=int, default=0)
...
def wait_for_workers(controller_info):
    while True:
        ...
        try:
            ...
            thread = threading.Thread(target=handle_connection, args=(controller_info, conn), daemon=True)
            thread.start()
...
def send_job(controller_info, conn, chunk):
    payload = {
        ...
        "chunk_start": chunk[0],
        "chunk_end": chunk[1],
        "checkpoint": controller_info.checkpoint,
        "algorithm": controller_info.data["algorithm"],
        "options": controller_info.data["options"],
        "salt": controller_info.data["salt"],
        "password": controller_info.data["password"]
    }
...
def broadcast_stop(controller_info, conn):
    with controller_info.worker_lock:
        ...
        if conn in controller_info.worker_chunks:
            del controller_info.worker_chunks[conn]
        if conn in controller_info.worker_progress:
            del controller_info.worker_progress[conn]
...
def get_chunk(controller_info):
    if controller_info.requeue_chunks:
        return controller_info.requeue_chunks.popleft()
    chunk_s = controller_info.chunk_start
    chunk_e = controller_info.chunk_start + controller_info.chunk_size
    controller_info.chunk_start = chunk_e  # fixed increment bug
...
def print_report(controller_info):
    ...
    total_chunks = max(controller_info.total_chunks, 1)
    print(f"Chunk Assign Avg: {controller_info.chunk_assign_time / total_chunks:.6f}s")
    if controller_info.result:
        r = controller_info.result[0]
        print(f"\nWorker {r['worker']}:")
...
def main():
    ...
    if controller_info.hb_interval > 0:
        threading.Thread(target=heartbeat_loop, args=(controller_info,), daemon=True).start()
